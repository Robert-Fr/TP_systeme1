...
int main(int argc, char *argv[]) {
    ...
    /* Algo */
    struct timeval *tp1=malloc(sizeof(struct timeval));
    struct timeval  *tp2=malloc(sizeof(struct timeval));
	struct timeval * tp_total=malloc(sizeof(struct timeval));
	
	struct rusage* info_ressource_total=malloc(sizeof( struct rusage));
	struct rusage* info_ressource1=malloc(sizeof( struct rusage));
	struct rusage* info_ressource2=malloc(sizeof( struct rusage));
	
    if(temps==1)
		gettimeofday(tp1,NULL);
	if(ressources==1)
		getrusage(RUSAGE_SELF,info_ressource1);
	...
	if(ressources ==1 ){
		 ...
		 info_ressource_total->ru_utime.tv_sec=info_ressource2->ru_utime.tv_sec-info_ressource1->ru_utime.tv_sec;
